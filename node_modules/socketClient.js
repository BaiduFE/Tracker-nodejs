...
function clientCode(){
    var _tracker_ =
        ...
        _fnEnd_ = function(){};
    void function(){
        if(window.parent !== window){return;}//对iframe不做处理
        var conf = $;
        var socket = io.connect('http://' + conf.socketServer.host + ':' + conf.socketServer.port),
            ...
            trackerTaskId = location.search.match(/trackerTaskId=([^&]+)/),
            timer, pageLoadTime;
        trackerTaskId = trackerTaskId && trackerTaskId[1];
...
//                    if(window.parent !== window){return;}
        ...
        _fnStart_ = function(codeId, ident){
            tasks.push({
                type: 'fnStart',
                ident: codeId,
                fnId: ident,
